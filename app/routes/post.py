...
def api_post():
    # * Es la infomracion que nos envia el cliente
    # * recuerden que con .json transforma la data y puedo leerle desde py
    # * para este caso solo requiero 2 cosas body, user_id
    # ! {"body": "Texto del post", "user_id": 4}
    client_post_information = request.json
    new_post = Post(
        body=client_post_information["body"], user_id=client_post_information["user_id"]
    )
    db.session.add(new_post)
    db.session.commit()
